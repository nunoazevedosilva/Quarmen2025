...
class SbnSolver:
    ...
    def linear_step1d(self, dz):

        self.envelope.profile = af.signal.fft(self.envelope.profile)
        self.envelope.profile = self.envelope.profile*af.exp(- 1j* dz * self.k2 * self.l_factor) #/ (self.pmesh.Nx * self.pmesh.Ny * self.pmesh.Nt)
        self.envelope.profile = af.signal.ifft(self.envelope.profile)


        self.envelope2.profile = af.signal.fft(self.envelope2.profile)
        self.envelope2.profile = self.envelope2.profile*af.exp(- 1j* dz * self.k2 * self.l_factor) #/ (self.pmesh.Nx * self.pmesh.Ny * self.pmesh.Nt)
        self.envelope2.profile = af.signal.ifft(self.envelope2.profile)
    ...
    def push1d(self, stride):            
            
        # First a half linear step
        self.linear_step1d(self.pmesh.dz / 2)

        for step in range(stride - 1):
            self.nonlinear_step(self.pmesh.dz)
            self.linear_step1d(self.pmesh.dz)
